...
firebase.auth().onAuthStateChanged((user) => {
    if (user) {
      ...
      firebase.database().ref(`users/${uid}/teamsOwned/${teamid}/question`).on("value", (data)=>{
      if(data.val()==null){
        firebase.database().ref(`users/${uid}/teamsOwned/${teamid}/question`).set(
    {
        q1,
        q2,
        q3
...
let GetDataFromFireBase = ()=>{
        ...
        firebase.database().ref(`users/${uid}/teamsOwned/${teamid}`).on("value",(data)=>{
